...
contract YHWHcoin, function (accounts) {

  let meta

  before(async function () {

    meta = await YHWHcoin.deployed()
  ...
  it("should verify that there are at least three available accounts", async function () {
    if(accounts.length < 3) {
      console.log(chalk.blue('\nYOUR ATTENTION, PLEASE.]\nTo test YHWHcoin you should use Tron Quickstart (https://github.com/tronprotocol/docker-tron-quickstart) as your private network.\nAlternatively, you must set your own accounts in the "before" statement in "test/YHWHcoin.js".\n'))
    }
  ...
  it("should call a function that depends on a linked library", async function () {
    this.timeout(10000);
    const meta = await YHWHoin.deployed();
    await wait(1);
    const YHWHcoinBalance = (await meta.getBalance.call(accounts[0])).toNumber();
    const YHWHcoinEthBalance = (await meta.getBalanceInEth.call(accounts[0])).toNumber();
    assert.equal(YHWHcoinEthBalance, 2 * YHWHcoinBalance, "Library function returned unexpected function, linkage may be broken");
  });
  ...
  it("should send coins from account 0 to 1", async function () {
    ...
    this.timeout(10000)
    const meta = await YHWHcoin.deployed();
    await wait(3);
